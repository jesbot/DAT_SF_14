...
from sklearn.preprocessing import LabelEncoder, roc_curve
le = LabelEncoder()
...
labelDF = pd.concat(([dnnTrain['CorrectLabel'], dnnTrain['Classified']]),\
	axis=1,keys=('CorrectLabel','NNLabel'))

# Tokenize it
for col in labelDF:
	le.fit(labelDF[col])
	labelDF[col] = le.transform(labelDF[col])
...
def plot_confusion_matrix(cm, title='Confusion Matrix AlexNet Bias', cmap=plt.cm.Reds):
...
print cmNormalized
plot_confusion_matrix(cmNormalized,'Normalized Confusion Matrix AlexNet Bias Sample')
